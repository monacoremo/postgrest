...
allColumns tabs =
 ...
 where
  sql = [q|
    select distinct
        relnamespace.nspname::text as schema,
        pg_class.relname::text as table_name,
        pg_attribute.attname::text as name,
        pg_description.description::text as description,
        pg_attribute.attnum::integer as position,
        
        (not (
            pg_attribute.attnotnull
            or pg_type.typtype = 'd' -- domain
            and pg_type.typnotnull
        ))::bool as nullable,

        case
            when pg_type.typtype = 'd' then    
                case
                    when basetype.typelem <> 0::oid and basetype.typlen = (-1) then
                        'ARRAY'
                    when basetypenamespace.nspname = 'pg_catalog'::name then
                        format_type(pg_type.typbasetype, null)
                    else
                        format_type(pg_attribute.atttypid, pg_attribute.atttypmod)
                end
            else
                case
                    when pg_type.typelem <> 0::oid and pg_type.typlen = (-1) then
                        'ARRAY'
                    when typnamespace.nspname = 'pg_catalog'::name then
                        format_type(pg_attribute.atttypid, null)
                    else
                        format_type(pg_attribute.atttypid, pg_attribute.atttypmod)
                end
        ...
        (
            pg_class.relkind = any (array['r', 'v', 'f']) 
            and pg_column_is_updatable(pg_class.oid::regclass, pg_attribute.attnum, false)
        )::bool as updateable,
        
        information_schema._pg_char_max_length(typid, typmod)::integer as max_len,
        information_schema._pg_numeric_precision(typid, typmod)::integer as precision,
        pg_get_expr(pg_attrdef.adbin, pg_attrdef.adrelid)::text as default_value,
        array_to_string(enum.vals, ',')::text as enum
    from
        pg_attribute
        join pg_class 
            on pg_class.oid = pg_attribute.attrelid
        join pg_namespace relnamespace 
            on relnamespace.oid = pg_class.relnamespace
        join pg_type 
            on pg_type.oid = pg_attribute.atttypid
        join pg_namespace typnamespace 
            on typnamespace.oid = pg_type.typnamespace 
        ...
        left join pg_type basetype 
            on basetype.oid = pg_type.typbasetype
        left join pg_namespace basetypenamespace 
            on basetypenamespace.oid = basetype.typnamespace,
        lateral (
            select array_agg(pg_enum.enumlabel order by pg_enum.enumsortorder) as vals
            from pg_enum
            where pg_type.oid = pg_enum.enumtypid 
        ) enum,
        information_schema._pg_truetypid(pg_attribute.*, pg_type.*) as typid,
        information_schema._pg_truetypmod(pg_attribute.*, pg_type.*) as typmod
    where
        not pg_is_other_temp_schema(relnamespace.oid)
        and pg_attribute.attnum > 0
        and not pg_attribute.attisdropped
        and pg_class.relkind = any (array['r', 'v', 'f', 'm'])
        and relnamespace.nspname not in ('pg_catalog', 'information_schema')
    order by
        schema, table_name, position |]
