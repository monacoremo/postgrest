...
import PostgREST.Types            (Column (..), DbStructure,
                                   ...
                                   Table (..), dbPrimaryKeys,
                                   proxyHost, proxyPath, proxyPort,
                                   proxyScheme, tableCols, tableName,
...
proxyUri conf =
  case pickProxy $ toS <$> configOpenApiServerProxyUri conf of
    Just proxy ->
      ( proxyScheme proxy
      , proxyHost proxy
      , proxyPort proxy
    ...
    Nothing ->
      ("http"
      , configServerHost conf
      , toInteger $ configServerPort conf
      , "/"
      )
